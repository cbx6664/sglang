...
def setup_environment() -> None:
    ...
    os.environ["CUSTOM_EXPERT_ALLOCATION"] = ENV["custom_expert_allocation"]
    os.environ["MODEL_PATH"] = CONFIG["model_path"]
    os.environ["LOG_ALL"] = ENV["log_all"]
    os.environ["LOG_DIR"] = ENV["log_dir"]
    os.environ["NUM_EXPERTS"] = ENV["num_experts"]
    os.environ["EXPERT_ALLOCATION_FILE_PATH"] = ENV["expert_allocation_file_path"]
...
def get_engine_instance() -> Engine:
    ...
    server_args = ServerArgs(
        ...
        enable_ep_moe=CONFIG["enable_ep_moe"],
...
def load_sample_requests() -> List[Tuple[str, int, int]]:
    ...
    for _, request in processed_data.iterrows():
        prompts.append((
            request['input'], 
            request['tok_input_len'], 
            request['tok_ref_output_len']
        ))
...
def save_outputs_to_json(outputs: Any) -> None:
    ...
    path = os.path.join(ENV["log_dir"], "outputs.json")
...
def profile_run_sglang(requests: List[Tuple[str, int, int]], sampling_params: Dict[str, Any]) -> None:
...
def run_sglang(requests: List[Tuple[str, int, int]], sampling_params: Dict[str, Any]) -> Tuple[List[str], List[str], float]:
    ...
    # Log basic stats
    avg_output_len = sum(len(text) for text in output_texts) / len(output_texts)
    logger.info(f"Inference time: {elapsed_time:.2f} seconds")
    logger.info(f"Average output length: {avg_output_len:.1f} characters")
    logger.info(f"Sample output: {output_texts[0][:100]}...")
    
    save_outputs_to_json(outputs)
    engine.shutdown()
    
    return prompts, output_texts, elapsed_time


def calculate_throughput(requests: List[Tuple[str, int, int]], 
                        prompts: List[str], 
                        outputs: List[str], 
    ...
    # Log results
    logger.info(f"Throughput: {req_throughput:.2f} requests/s, "
...
def main(enable_profiling: bool = False) -> None:
    ...
    if enable_profiling:
        ...
        logger.info("Running standard inference benchmark")
        prompts, outputs, elapsed_time = run_sglang(requests, DEFAULT_SAMPLING_PARAMS)
        calculate_throughput(requests, prompts, outputs, elapsed_time)


if __name__ == "__main__":
    main(enable_profiling=True)  # Set to False to disable profiling